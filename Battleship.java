...
public class Battleship extends ConsoleProgram
{
    ...
    private void askForGuess(Player current, Player opp){
        ...
        boolean hit = opp.recordOpponentGuess(row,col);
    ...
    private boolean shipFits(Ship s, int row, int col, int direction){
        if (direction == 0){ //horizontal
            return col + s.getLength() <= 10;
        }else{
            return row + s.getLength() <= 10;
        }
    }
    
    private void setUpPlayerShips(){
        Ship[] ships = p1.getShips();
        
        for (int i = 0; i < ships.length; i++){
            Ship s = ships[i];
            
            System.out.println("Place ship of length " + s.getLength());
            
            int row = readInt("Row: ");
            int col = readInt("Col: ");
            int direction = readInt("H = 0, V = 1: ");
            
            while (!shipFits(s,row,col,direction)){
                System.out.println("Ship does not fit!");
                
                row = readInt("Row: ");
                col = readInt("Col: ");
                direction = readInt("H = 0, V = 1: ");
            }
